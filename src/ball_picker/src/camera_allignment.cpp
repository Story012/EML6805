...
cv::Mat Camera_Matrix;           // From calibration_data.xml
cv::Mat Distortion_Coefficients; // From calibration_data.xml

Camera_Matrix = (Mat1d(3, 3) << 1874.78176,0,969.74004,0,1922.61106,647.01136,0,0,1);
Distortion_Coefficients = (Mat1d(1, 5) <<-0.406362, 0.071205, 0.016847, 0.012809, 0.000000);
...
cv::Point traceBall(const cv::Mat &image) {
    ...
    cv::Scalar lower(hue - 1, saturation, 100);                                          // !!!!!!
    cv::Scalar upper(hue + 1, 255, 255);                                            // !!!!!!
    // Create mask
...
int main(int argc, char **argv) {
    ...
    while( true ) {
		...
		std::vector<int> point = processImage(image); // Implement our image processing
